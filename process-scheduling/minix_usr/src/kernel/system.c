...
FORWARD _PROTOTYPE( int do_getpnr, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_setschedparams, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_movusrq, (message *m_ptr) );
FORWARD _PROTOTYPE ( int do_printqueue, (message *m_ptr));
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_GETPNR:	r = do_getpnr(&m);	break;
	    case SYS_SETSCHEDPARAMS:	r = do_setschedparams(&m);	break;
	    case SYS_MOVEUSERQUEUE:     r = do_movusrq(&m);	break;
	    case SYS_PRINTQUEUE:	r = do_printqueue(&m);   break;
	    default:		r = E_BAD_FCN;
	}
...
PRIVATE int do_setschedparams(m_ptr)
register message *m_ptr;
{
  if (m_ptr->m1_i1 == 1)
  	RUN_LIMIT1 = m_ptr->m1_i2;
  else if(m_ptr->m1_i1 == 2)
  	WAIT_LIMIT2 = m_ptr->m1_i2;
  else if(m_ptr->m1_i1 == 3)
  	WAIT_LIMIT3 = m_ptr->m1_i2;
  else
  	QUEUE2_RATE = m_ptr->m1_i2;
}

PRIVATE int do_movusrq(m_ptr)
register message *m_ptr;
{
     if (m_ptr->m1_i2 > 2 && m_ptr->m1_i2 < 7)
     {
     	proc[m_ptr->m1_i1].p_priority = m_ptr ->m1_i2;
     	proc[m_ptr->m1_i1].p_class = m_ptr ->m1_i2;
     	
     }
}

PRIVATE int do_printqueue(m_ptr)
register message *m_ptr;
{
	...
	{
		current = rdy_head[USER_IO_Q];
	while (current != rdy_tail[USER_IO_Q]->p_nextready)
	{
		printf("[ Pid: %d ] ", current->p_pid);
	...
	{
		current = rdy_head[USER_INT_Q];
	while (current != rdy_tail[USER_INT_Q]->p_nextready)
	{
		printf("[ Pid: %d ] ", current->p_pid);
		current = current-> p_nextready;
	}
	...
	{
		current = rdy_head[USER_NUM_Q];
	while (current != rdy_tail[USER_NUM_Q]->p_nextready)
	{
		printf("[ Pid: %d ] ", current->p_pid);
		current = current-> p_nextready;
	}
...
#if (CHIP == INTEL)
  old_ldt_sel = rpc->p_ldt_sel;	/* stop this being obliterated byfp_ copy */
#endif
