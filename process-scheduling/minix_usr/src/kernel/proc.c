 ...
 */
  ...
  if ( (rp = rdy_head[USER_IO_Q]) != NIL_PROC) {
	proc_ptr = rp;
	proc_ptr->p_uptime = 0;
	bill_ptr = rp;
	return;
  }
  if ( (rp = rdy_head[USER_INT_Q]) != NIL_PROC) {
	proc_ptr = rp;
	proc_ptr->p_uptime = 0;
	bill_ptr = rp;
	return;
  }
  if ( (rp = rdy_head[USER_NUM_Q]) != NIL_PROC) {
	proc_ptr = rp;
	proc_ptr->p_uptime = 0;
	bill_ptr = rp;
	return;
 ...
 */
   ...
   * bound processes.)
   */
  if (isuseriop(rp)) {		/* others are similar */
	if (rdy_head[USER_IO_Q] != NIL_PROC)
		rdy_tail[USER_IO_Q]->p_nextready = rp;
	else
		rdy_head[USER_IO_Q] = rp;
	rdy_tail[USER_IO_Q] = rp;
	rp->p_nextready = NIL_PROC;
	return;
  }
  if (isuserintp(rp)) {		/* others are similar */
	if (rdy_head[USER_INT_Q] != NIL_PROC)
		rdy_tail[USER_INT_Q]->p_nextready = rp;
	else
		rdy_head[USER_INT_Q] = rp;
	rdy_tail[USER_INT_Q] = rp;
	rp->p_nextready = NIL_PROC;
	return;
  }
  if (rdy_head[USER_NUM_Q] == NIL_PROC)
	rdy_tail[USER_NUM_Q] = rp;
  rp->p_nextready = rdy_head[USER_NUM_Q];
  rdy_head[USER_NUM_Q] = rp;
}
...
/* A process has blocked. */
  ...
  if (istaskp(rp)) {
  ...
  else if (isuseriop(rp)) {
	if ( (xp = rdy_head[USER_IO_Q]) == NIL_PROC) return;
	if (xp == rp) {
		rdy_head[USER_IO_Q] = xp->p_nextready;
#if (CHIP == M68000)
		if (rp == proc_ptr)
#endif
		pick_proc();
	...
	qtail = &rdy_tail[USER_IO_Q];
  }
  else if (isuserintp(rp)) {
	if ( (xp = rdy_head[USER_INT_Q]) == NIL_PROC) return;
	if (xp == rp) {
		rdy_head[USER_INT_Q] = xp->p_nextready;
#if (CHIP == M68000)
		if (rp == proc_ptr)
#endif
		pick_proc();
	...
	qtail = &rdy_tail[USER_INT_Q];
   }
  else {
	if ( (xp = rdy_head[USER_NUM_Q]) == NIL_PROC) return;
	if (xp == rp) {
		rdy_head[USER_NUM_Q] = xp->p_nextready;
#if (CHIP == M68000)
		if (rp == proc_ptr)
#endif
		pick_proc();
 ...
 */
  if (rdy_head[USER_IO_Q] != NIL_PROC)
  {
  	rdy_tail[USER_IO_Q]->p_nextready = rdy_head[USER_IO_Q];
  	rdy_tail[USER_IO_Q] = rdy_head[USER_IO_Q];
  	rdy_head[USER_IO_Q] = rdy_head[USER_IO_Q]->p_nextready;
  	rdy_tail[USER_IO_Q]->p_nextready = NIL_PROC;
  	pick_proc();	
  }
  else if (rdy_head[USER_INT_Q] != NIL_PROC)
  {
  	rdy_tail[USER_INT_Q]->p_nextready = rdy_head[USER_INT_Q];
  	rdy_tail[USER_INT_Q] = rdy_head[USER_INT_Q];
  	rdy_head[USER_INT_Q] = rdy_head[USER_INT_Q]->p_nextready;
  	rdy_tail[USER_INT_Q]->p_nextready = NIL_PROC;
  	pick_proc();	
  }
  else
  {
  	if (rdy_head[USER_NUM_Q] == NIL_PROC) return;
  	/* One or more user processes queued. */
  	rdy_tail[USER_NUM_Q]->p_nextready = rdy_head[USER_NUM_Q];
  	rdy_tail[USER_NUM_Q] = rdy_head[USER_NUM_Q];
  	rdy_head[USER_NUM_Q] = rdy_head[USER_NUM_Q]->p_nextready;
  	rdy_tail[USER_NUM_Q]->p_nextready = NIL_PROC;
  	pick_proc();
  }
}
