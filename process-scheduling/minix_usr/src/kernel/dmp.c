...
/* Proc table dump */
  ...
  printf("\n-nr -prio -class -time --pid -flag -user --sys-- -text- -data- -size- -recv- command\n");
  ...
  for (rp = oldrp; rp < END_PROC_ADDR; rp++) {
	...
	printf("%3d %4d %4d %6d ", (int)((rp-BEG_PROC_ADDR)), rp->p_priority,rp->p_class, rp->p_uptime);
	if (rp->p_pid == 0) {
		printf("(%4d)", proc_number(rp));
	} else {
		printf("%4d", rp->p_pid);
	}
	/* %5lx %6lx */
