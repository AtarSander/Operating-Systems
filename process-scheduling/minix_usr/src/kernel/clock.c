 ...
 */
  ...
  if (bill_ptr -> p_class != 5)
  {
  if (--sched_ticks == 0) {
	if (bill_ptr == prev_ptr) lock_sched();	/* process has run too long */
	sched_ticks = SCHED_RATE;		/* reset quantum */
	if (bill_ptr -> p_class == 4)
		sched_ticks = QUEUE2_RATE;
 ...
 */
  ...
  for(rp = BEG_PROC_ADDR; rp < END_PROC_ADDR; rp++)
  {
  	if (rp->p_priority == 4 && rp->p_uptime > WAIT_LIMIT2)
  		{
  		lock_unready(rp);
  		rp-> p_priority = 3;
  		rp->p_class = 3;
  	...
  	if (rp->p_priority == 5 && rp->p_uptime > WAIT_LIMIT3)
  		{
  		lock_unready(rp);
  		rp->p_priority = 4;
  		rp->p_class = 4;
  ...
  if (next_alarm <= now ||
      sched_ticks == 1 &&
      bill_ptr == prev_ptr &&
      rdy_head[USER_NUM_Q] != NIL_PROC) {
	interrupt(CLOCK);
	return 1;	/* Reenable interrupts */
  }
  if (bill_ptr -> p_class == 3)
  {
       if (--sched_ticks == 0) {
       if (bill_ptr == prev_ptr) lock_sched();	/* process has run too long */
	sched_ticks = SCHED_RATE;		/* reset quantum */
