...
PUBLIC int do_get_pnr()
{
	int r;
	r = _taskcall(SYSTASK, SYS_GETPNR, &mm_in);
	mp -> mp_reply = mm_in;
	return r;
}

PUBLIC int do_set_sched_params()
{
	return _taskcall(SYSTASK, SYS_SETSCHEDPARAMS, &mm_in);
}

PUBLIC int do_move_user_queue()
{
	return _taskcall(SYSTASK, SYS_MOVEUSERQUEUE, &mm_in);
}

PUBLIC int do_print_queue()
{
	return _taskcall(SYSTASK, SYS_PRINTQUEUE, &mm_in);
}
