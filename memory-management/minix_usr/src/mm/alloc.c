...
PUBLIC int do_hole_map(void)
{
	...
	unsigned int temp_buffer[1024];
	size_t max = mm_in.m1_i1;
	unsigned int *buf_ptr = mm_in.m1_p1;
	pid_t pidd = mm_in.m_source;
	int i=0;
	
	hp = hole_head;
	while (hp != NIL_HOLE && i<(max/4)-2)
	{
		temp_buffer[i] = hp->h_len;
		temp_buffer[i+1] = hp->h_base;
		i+=2;
		hp = hp->h_next;
	}
	temp_buffer[i] = 0;
	sys_copy(MM_PROC_NR, D, (phys_bytes) temp_buffer,
			pidd, D, (phys_bytes) buf_ptr,
			(phys_bytes) max);
	return i/2;
}
/*===========================================================================*
  ...
  else
  {
	...
	while (hp != NIL_HOLE) 
	{
	  ...
	  if (max_hole->h_len >= clicks) {
				...
				/* Delete the hole if used up completely. */
				if (max_hole->h_len == 0) del_slot(prev_ptr, max_hole);
